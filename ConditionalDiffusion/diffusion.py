...
class DenoiseDiffusion:
    ...
    def q_xt_x0(self, x0: torch.Tensor, t: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # 即为本步中我们要求的mean和var
        # ----------------------------------------------------------------
        mean = Tools.gather(self.alpha_bar, t) ** 0.5 * x0
        var = 1 - Tools.gather(self.alpha_bar, t)
    ...
    def p_sample(self, xt: torch.Tensor, t: torch.Tensor, y):
        """
        ...
        alpha_bar =  Tools.gather(self.alpha_bar, t)       
        alpha = Tools.gather(self.alpha, t)
        eps_coef = (1 - alpha) / (1 - alpha_bar) ** .5
        mean = 1 / (alpha ** 0.5) * (xt - eps_coef * eps_theta)
        var = Tools.gather(self.sigma2, t)
        eps = torch.randn(xt.shape, device=xt.device)
 
        return mean + (var ** .5) * eps

    def loss(self, x0: torch.Tensor,y, noise: Optional[torch.Tensor] = None):
        """
        1. 随机抽取一个time_step t
